        ...
        if message["role"] == "assistant":
            ...
            with col1:
                if st.button("👍", key=f"thumbs_up_{i}", help="Was this Helpful?"):
                    ...
                    # log_to_google_sheets(pdf_name, message["content"], "Thumbs Up")  # Logging disabled for debugging
            with col2:
                if st.button("👎", key=f"thumbs_down_{i}", help="Was this Helpful?"):
                    st.session_state.feedback[message['content']] = "Thumbs Down"
                    # log_to_google_sheets(pdf_name, message["content"], "Thumbs Down")  # Logging disabled for debugging
    ...
    with st.chat_message("assistant"):
        ...
        with col1:
            if st.button("👍", key=f"thumbs_up_{len(st.session_state.messages)}", help="Was this Helpful?"):
                ...
                # log_to_google_sheets(pdf_name, response, "Thumbs Up")  # Logging disabled for debugging
        with col2:
            if st.button("👎", key=f"thumbs_down_{len(st.session_state.messages)}", help="Was this Helpful?"):
                ...
                # log_to_google_sheets(pdf_name, response, "Thumbs Down")  # Logging disabled for debugging
